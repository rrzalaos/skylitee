...
export function BarRow({ label, pct, value, color = "orange" }: { label: string; pct: number; value: string; color?: BarColor }) {
  return (
    <div className="flex items-center gap-2 mb-2">
      <div className="text-[13px] text-[#71717A] dark:text-[#A1A1AA] w-28 text-right shrink-0 truncate font-medium">{label}</div>
      ...
      <div className="text-[13px] text-[#18181B] dark:text-[#F4F4F5] font-semibold w-16 shrink-0">{value}</div>
    </div>
  );
}
