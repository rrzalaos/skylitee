    ...
    <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${scrolled ? "bg-white/90 backdrop-blur-md shadow-sm border-b border-black/[0.06]" : "bg-transparent"}`}>
      <div className="max-w-6xl mx-auto px-4 sm:px-6 h-16 sm:h-18 flex items-center justify-between">
        ...
        <div className="hidden md:flex items-center gap-3">
          <Link href="/install" className="px-5 py-2.5 bg-[#F97316] hover:bg-[#EA580C] text-white text-[14px] font-bold rounded-lg transition-all shadow-[0_0_16px_rgba(249,115,22,0.35)] hover:shadow-[0_0_24px_rgba(249,115,22,0.5)] hover:-translate-y-0.5">
            Install Free →
          </Link>
        </div>
      ...
      {open && (
        <div className="md:hidden bg-white border-t border-black/[0.06] px-5 py-5 flex flex-col gap-4 shadow-lg">
          ...
          <Link href="/install" onClick={() => setOpen(false)} className="px-4 py-3 bg-[#F97316] text-white text-[15px] font-bold rounded-lg text-center mt-1">
            Install Free →
          </Link>
        </div>
      )}
