    ...
    <div>
      <div className="flex items-start justify-between mb-3">
        <div>
          ...
          <p className="text-[12px] text-[#686864] mt-0.5">P&L · unit economics · LTV · break-even analysis</p>
        </div>
      ...
      <div className="grid grid-cols-2 gap-2">
        <Card>
          ...
          {[
            ...
            <div key={i} className={`flex justify-between items-center py-1.5 border-b border-black/[0.06] last:border-0 ${row.large ? "border-t border-black/[0.09] pt-2" : ""}`}>
              <span className={`text-[11px] ${row.sub ? "pl-3 text-[#686864] text-[10px]" : ""} ${row.bold ? "font-semibold" : ""}`}>
                {row.label}
              </span>
              <span className={`text-[11px] font-medium ${row.positive ? "text-[#0d6b4f]" : "text-[#d94040]"} ${row.bold ? "font-semibold" : ""} ${row.large ? "text-[13px]" : ""}`}>
        ...
        <div>
          <Card className="mb-2">
            ...
            <div className="mb-3">
              <div className="text-[10px] text-[#686864] mb-1">COGS % <span className="font-semibold text-[#181816]">{cogs}%</span></div>
              <input type="range" min={20} max={60} step={1} value={cogs} onChange={e => setCogs(+e.target.value)} className="w-full accent-[#17a773]" />
            </div>
            <div className="mb-3">
              <div className="text-[10px] text-[#686864] mb-1">Logistics % <span className="font-semibold text-[#181816]">{logistics}%</span></div>
            ...
            <div className="bg-[#f7f7f5] rounded-lg px-3 py-2.5">
              <div className="text-[10px] text-[#686864]">Break-even ROAS:</div>
