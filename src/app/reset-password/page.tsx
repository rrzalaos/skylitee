...
function ResetForm() {
  ...
  if (!token) {
    return (
      <div className="text-center py-4">
        <p className="text-[14px] text-[#EF4444]">Invalid reset link.</p>
        <Link href="/forgot-password" className="text-[14px] text-[#F97316] mt-3 inline-block">Request a new one →</Link>
      </div>
    );
  }
  ...
  return done ? (
    <div className="text-center py-4">
      ...
      <p className="text-[14px] text-white/50">Redirecting to login...</p>
    </div>
  ) : (
    <>
      <h1 className="text-[20px] font-bold text-white mb-1">Set new password</h1>
      <p className="text-[14px] text-white/50 mb-6">Enter your new password below.</p>

      <form onSubmit={handleSubmit} className="space-y-4">
        <div>
          <label className="text-[13px] font-semibold text-white/60 mb-1.5 block uppercase tracking-[0.08em]">New Password</label>
