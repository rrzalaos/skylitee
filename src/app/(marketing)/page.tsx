...
import { Check, BarChart2, Target, Zap, ShoppingBag, Search, TrendingUp, ArrowRight } from "lucide-react";
...
function DashboardMockup() {
  ...
  return (
    <div className="rounded-2xl overflow-hidden shadow-[0_40px_80px_rgba(0,0,0,0.7),0_0_0_1px_rgba(255,255,255,0.08)]">
      ...
      <div className="bg-[#111113] flex" style={{ minHeight: 290 }}>
...
export default function HomePage() {
  return (
    <>
      ...
      <section className="relative bg-[#0A0A0A] text-white overflow-hidden">
        ...
        <div className="relative max-w-6xl mx-auto px-4 sm:px-6 pt-24 pb-16">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            <div>
              <div className="inline-flex items-center gap-2 bg-[#F97316]/10 border border-[#F97316]/20 text-[#F97316] text-[11px] font-bold px-3 py-1.5 rounded-full mb-6">
                <Zap size={11} fill="#F97316" /> Live on Shopify App Store
              </div>
              <h1 className="text-[44px] sm:text-[52px] font-black leading-[1.1] mb-6">
              ...
              <p className="text-[16px] text-[#A1A1AA] leading-relaxed mb-8 max-w-md">
              ...
              <div className="flex flex-col sm:flex-row gap-3 mb-10">
                <Link href="/install"
                  className="px-6 py-3.5 bg-[#F97316] hover:bg-[#EA580C] text-white font-bold rounded-xl text-[14px] transition-all shadow-[0_0_24px_rgba(249,115,22,0.4)] hover:shadow-[0_0_36px_rgba(249,115,22,0.6)] hover:-translate-y-0.5 text-center">
                  Start free — 14 days →
                </Link>
                <Link href="/features"
                  className="px-6 py-3.5 bg-white/[0.06] hover:bg-white/[0.10] border border-white/10 text-white font-bold rounded-xl text-[14px] transition-all text-center">
                  See all features
                </Link>
              </div>
              <div className="flex items-center gap-6">
                {[{ v: "₹0", l: "to set up" }, { v: "14d", l: "free trial" }, { v: "5min", l: "to connect" }].map(s => (
                  <div key={s.l}>
                    <div className="text-[22px] font-black text-white">{s.v}</div>
                    <div className="text-[11px] text-[#52525B] font-medium">{s.l}</div>
                  </div>
                ))}
              </div>
            </div>
            <div className="relative">
              ...
              <div className="absolute -top-4 -right-4 bg-[#22C55E] text-white text-[11px] font-black px-3 py-1.5 rounded-full shadow-lg">
                Live data ↑
              </div>
              <div className="absolute -bottom-4 -left-4 bg-[#1C1C1E] border border-white/10 text-white text-[11px] font-bold px-3 py-2 rounded-xl shadow-xl">
        ...
        <div className="relative border-t border-white/[0.05] py-8 px-4">
          <div className="max-w-4xl mx-auto">
            <div className="text-center text-[11px] font-bold uppercase tracking-[0.12em] text-[#3F3F46] mb-6">Connects with</div>
            <div className="flex items-center justify-center gap-8 flex-wrap">
              {platforms.map(p => (
                <div key={p.name} className="flex items-center gap-2 opacity-60 hover:opacity-100 transition-opacity">
                  ...
                  <span className="text-[13px] font-semibold text-[#71717A]">{p.name}</span>
                </div>
              ))}
            </div>
          </div>
      ...
      {/* ── Feature sections ── */}
      {features.map((f, idx) => (
        <section key={f.tag} className={`py-20 px-4 ${idx % 2 === 0 ? "bg-white" : "bg-[#F5F5F4]"}`}>
          <div className="max-w-5xl mx-auto">
            <div className={`grid grid-cols-1 lg:grid-cols-2 gap-12 items-center ${idx % 2 !== 0 ? "lg:flex-row-reverse" : ""}`}
              ...
              <div style={{ direction: "ltr" }}>
                <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full text-[11px] font-bold mb-5 border"
                  style={{ background: `${f.tagColor}10`, color: f.tagColor, borderColor: `${f.tagColor}30` }}>
                  <f.icon size={11} /> {f.tag}
                </div>
                <h2 className="text-[32px] font-black text-[#18181B] leading-tight mb-4">{f.title}</h2>
                <p className="text-[15px] text-[#71717A] leading-relaxed mb-6">{f.desc}</p>
                <ul className="space-y-2.5">
                  {f.items.map(item => (
                    <li key={item} className="flex items-center gap-2.5 text-[13px] text-[#52525B] font-medium">
              ...
              <div style={{ direction: "ltr" }}>
                <div className="rounded-2xl bg-[#111113] border border-white/[0.08] overflow-hidden shadow-[0_20px_60px_rgba(0,0,0,0.3)]">
                  <div className="bg-[#1C1C1E] px-4 py-2.5 flex items-center gap-2 border-b border-white/[0.05]">
                    ...
                    <span className="text-[10px] text-[#48484A] ml-2 font-medium">{f.tag} · Skylitee</span>
                  </div>
                  <div className="p-4 space-y-2">
                    {f.mockupLines.map(m => (
                      <div key={m.label} className="flex items-center justify-between bg-[#1C1C1E] rounded-xl px-4 py-3 border border-white/[0.05]">
                        <span className="text-[12px] text-[#71717A] font-medium">{m.label}</span>
                        <div className="text-right">
                          <div className="text-[15px] font-black" style={{ color: m.color }}>{m.value}</div>
                          <div className="text-[10px] text-[#48484A]">{m.change}</div>
                        </div>
                      </div>
                    ))}
                  </div>
      ...
      <section className="py-24 px-4 bg-[#0A0A0A] text-white relative overflow-hidden">
        ...
        <div className="relative max-w-4xl mx-auto">
          <div className="text-center mb-14">
            <div className="text-[11px] font-bold uppercase tracking-[0.12em] text-[#F97316] mb-3">Simple setup</div>
            <h2 className="text-[36px] font-black">Up in 5 minutes, not 5 days</h2>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {steps.map((s, i) => (
              <div key={s.n} className="relative text-center">
                {i < steps.length - 1 && (
                  <div className="hidden md:block absolute top-8 left-[calc(50%+32px)] right-0 h-px bg-gradient-to-r from-[#F97316]/40 to-transparent" />
                )}
                <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-[#F97316] to-[#EA580C] flex items-center justify-center mx-auto mb-5 shadow-[0_0_24px_rgba(249,115,22,0.4)]">
                  <span className="text-white font-black text-[20px]">{s.n}</span>
                </div>
      ...
      <section className="py-20 px-4 bg-white">
        <div className="max-w-5xl mx-auto">
          <div className="text-center mb-12">
            <div className="text-[11px] font-bold uppercase tracking-[0.12em] text-[#F97316] mb-2">From real founders</div>
            <h2 className="text-[32px] font-black text-[#18181B]">Brand owners love Skylitee</h2>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {reviews.map(r => (
              <div key={r.name} className="rounded-2xl border border-black/[0.06] p-6 bg-white hover:shadow-lg hover:border-[#F97316]/20 transition-all">
                ...
                <p className="text-[14px] text-[#52525B] leading-relaxed mb-4">&ldquo;{r.text}&rdquo;</p>
                <div className="flex items-center gap-2 pt-2 border-t border-black/[0.05]">
                  <div className="w-8 h-8 rounded-full bg-[#F97316]/10 flex items-center justify-center text-[#F97316] font-black text-[13px]">
                    {r.name[0]}
                  </div>
                  <div>
                    <div className="text-[13px] font-bold text-[#18181B]">{r.name}</div>
                    <div className="text-[11px] text-[#A1A1AA]">{r.brand}</div>
                  </div>
                </div>
              </div>
            ))}
      ...
      <section className="py-20 px-4 bg-[#F5F5F4]">
        <div className="max-w-lg mx-auto text-center">
          ...
          <h2 className="text-[32px] font-black text-[#18181B] mb-2">One plan. Everything included.</h2>
          <p className="text-[15px] text-[#71717A] mb-8">No tiers. No hidden fees. Start free.</p>
          <div className="rounded-2xl border-2 border-[#F97316] bg-white p-8 shadow-[0_0_0_1px_rgba(249,115,22,0.2),0_16px_48px_rgba(249,115,22,0.12)]">
            ...
            <div className="flex items-end justify-center gap-1 mb-1">
              ...
              <span className="text-[15px] text-[#A1A1AA] mb-2">/mo</span>
            </div>
            <div className="text-[13px] text-[#22C55E] font-semibold mb-6">14-day free trial — no credit card</div>
            <ul className="text-left space-y-2 mb-6">
              {["Shopify full analytics", "Meta Ads dashboard", "Google GSC + GA4", "AI Insights & Chat", "Cohort retention & P&L"].map(feat => (
                <li key={feat} className="flex items-center gap-2 text-[13px] text-[#52525B]">
                  <Check size={12} className="text-[#22C55E] shrink-0" /> {feat}
                </li>
              ))}
            </ul>
            <Link href="/install" className="block w-full py-3.5 bg-[#F97316] hover:bg-[#EA580C] text-white font-bold rounded-xl text-[14px] text-center transition-all shadow-[0_0_20px_rgba(249,115,22,0.3)] hover:shadow-[0_0_32px_rgba(249,115,22,0.5)]">
              Start free trial →
            </Link>
          </div>
          <Link href="/pricing" className="inline-block mt-4 text-[13px] text-[#F97316] font-semibold hover:underline">
            Full pricing details <ArrowRight size={12} className="inline" />
          </Link>
        </div>
      </section>

      {/* ── Final CTA ── */}
      <section className="relative py-24 px-4 bg-[#0A0A0A] text-white text-center overflow-hidden">
        <div className="absolute inset-0 pointer-events-none">
        ...
        <div className="relative max-w-2xl mx-auto">
          <div className="inline-flex items-center gap-2 bg-[#F97316]/10 border border-[#F97316]/20 text-[#F97316] text-[11px] font-bold px-3 py-1.5 rounded-full mb-6">
            <TrendingUp size={11} /> Join hundreds of D2C brands
          </div>
          <h2 className="text-[40px] sm:text-[48px] font-black leading-tight mb-4">
            Ready to see your full brand picture?
          </h2>
          <p className="text-[16px] text-[#71717A] mb-10">
            14-day free trial. No credit card. Cancel anytime from Shopify admin.
          </p>
          <Link href="/install"
            className="inline-block px-10 py-4 bg-[#F97316] hover:bg-[#EA580C] text-white font-black rounded-xl text-[15px] transition-all shadow-[0_0_32px_rgba(249,115,22,0.4)] hover:shadow-[0_0_48px_rgba(249,115,22,0.6)] hover:-translate-y-0.5">
            Install on Shopify — Free to Start →
          </Link>
          <div className="flex items-center justify-center gap-6 mt-8 text-[12px] text-[#52525B]">
