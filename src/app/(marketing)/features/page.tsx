    ...
    <>
      ...
      <section className="bg-[#0A0A0A] text-white py-24 px-4 text-center relative overflow-hidden">
        ...
        <div className="relative max-w-3xl mx-auto">
          <div className="text-[11px] font-bold uppercase tracking-[0.12em] text-[#F97316] mb-3">Everything included</div>
          ...
          <p className="text-[16px] text-[#71717A] max-w-xl mx-auto mb-8">
            No tiers. No feature gating. Every single thing Skylitee does is in Skylitee Pro at $29/mo.
          </p>
          <Link href="/install" className="inline-block px-8 py-3.5 bg-[#F97316] hover:bg-[#EA580C] text-white font-bold rounded-xl text-[14px] transition-all shadow-[0_0_24px_rgba(249,115,22,0.4)] hover:shadow-[0_0_36px_rgba(249,115,22,0.6)]">
      ...
      {/* Platform sections */}
      {sections.map((s) => (
        <section key={s.platform} className={`py-20 px-4 ${s.darkBg ? "bg-[#0A0A0A]" : "bg-white"}`}>
          <div className="max-w-5xl mx-auto">
            <div className="mb-10">
              ...
              <h2 className={`text-[32px] font-black mb-2 ${s.darkBg ? "text-white" : "text-[#18181B]"}`}>{s.headline}</h2>
              <p className={`text-[15px] leading-relaxed max-w-xl ${s.darkBg ? "text-[#71717A]" : "text-[#71717A]"}`}>{s.sub}</p>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {s.features.map(f => (
                <div key={f.title}
                  className={`rounded-2xl p-5 border transition-all group hover:-translate-y-0.5 ${
                    s.darkBg
                  ...
                  <div className="w-10 h-10 rounded-xl flex items-center justify-center mb-4"
                    ...
                    <f.icon size={17} style={{ color: s.tagColor }} />
                  </div>
                  <div className={`text-[14px] font-bold mb-1.5 ${s.darkBg ? "text-white" : "text-[#18181B]"}`}>{f.title}</div>
                  <div className={`text-[13px] leading-relaxed ${s.darkBg ? "text-[#71717A]" : "text-[#71717A]"}`}>{f.desc}</div>
                </div>
              ))}
            </div>
          </div>
      ...
      <section className="py-20 px-4 bg-[#F5F5F4]">
        <div className="max-w-3xl mx-auto">
          <div className="text-center mb-10">
            <div className="text-[11px] font-bold uppercase tracking-[0.12em] text-[#F97316] mb-2">What you get</div>
            <h2 className="text-[32px] font-black text-[#18181B]">Full access. Day one.</h2>
          </div>
          <div className="rounded-2xl bg-white border border-[#F97316]/20 shadow-[0_0_0_1px_rgba(249,115,22,0.1),0_12px_40px_rgba(249,115,22,0.08)] p-8">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {[
                ...
                <div key={feat} className="flex items-center gap-2.5 text-[13px] text-[#52525B]">
            ...
            <div className="mt-8 pt-6 border-t border-black/[0.06] flex flex-col sm:flex-row items-center justify-between gap-4">
              <div>
                <div className="text-[22px] font-black text-[#18181B]">$29<span className="text-[14px] font-normal text-[#A1A1AA]">/month</span></div>
                <div className="text-[12px] text-[#22C55E] font-semibold">14-day free trial — no credit card</div>
              </div>
              <Link href="/install" className="px-6 py-3 bg-[#F97316] hover:bg-[#EA580C] text-white font-bold rounded-xl text-[13px] transition-all shadow-[0_0_16px_rgba(249,115,22,0.3)]">
                Start free trial →
              </Link>
            </div>
          </div>
