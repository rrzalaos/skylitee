...
const reports = [
  {
    ...
    desc: "Your daily overview — revenue, ad spend, ROAS, GSC clicks and GA4 sessions on one screen.",
    tag: "All Platforms",
    tagColor: "#F97316",
    tagBg: "#F9731610",
    mockup: [
      { label: "Revenue", value: "₹4.2L", color: "#F97316" },
      { label: "ROAS", value: "3.8×", color: "#3B82F6" },
      { label: "Orders", value: "312", color: "#22C55E" },
    ],
    featured: true,
  },
  {
...
export default function ReportsPage() {
  ...
  return (
    <>
      {/* Hero */}
      <section className="bg-[#0A0A0A] text-white py-24 px-4 text-center relative overflow-hidden">
        ...
        <div className="relative max-w-3xl mx-auto">
          <div className="text-[11px] font-bold uppercase tracking-[0.12em] text-[#F97316] mb-3">12+ Reports included</div>
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 py-16">
        ...
        <div className="mb-10">
          <div className="rounded-2xl bg-[#0A0A0A] border border-white/[0.08] p-8 flex flex-col lg:flex-row gap-8 items-center shadow-[0_0_0_1px_rgba(249,115,22,0.15),0_20px_60px_rgba(0,0,0,0.4)]">
            <div className="flex-1">
              <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full text-[11px] font-bold mb-4 border"
                style={{ background: "#F9731615", color: "#F97316", borderColor: "#F9731630" }}>
                ★ Most popular — {featured.tag}
              </div>
              <h2 className="text-[28px] font-black text-white mb-3">{featured.title}</h2>
              <p className="text-[14px] text-[#71717A] leading-relaxed">{featured.desc}</p>
            </div>
            <div className="w-full lg:w-72 rounded-xl bg-[#161618] border border-white/[0.06] overflow-hidden">
              <div className="bg-[#1C1C1E] px-3 py-2 flex items-center gap-1.5 border-b border-white/[0.05]">
                <div className="w-2.5 h-2.5 rounded-full bg-[#FF5F57]" />
                <div className="w-2.5 h-2.5 rounded-full bg-[#FEBC2E]" />
                <div className="w-2.5 h-2.5 rounded-full bg-[#28C840]" />
                <span className="text-[10px] text-[#48484A] ml-2">Command Center</span>
              </div>
              <div className="p-3 space-y-2">
                {[
                  ...
                  <div key={m.label} className="flex items-center justify-between rounded-lg bg-[#111113] px-3 py-2.5 border border-white/[0.05]">
                    <span className="text-[11px] text-[#48484A]">{m.label}</span>
                    <div className="text-right">
                      <span className="text-[14px] font-black block" style={{ color: m.color }}>{m.value}</span>
                      <span className="text-[10px] text-[#22C55E]">{m.change}</span>
                    </div>
                  </div>
                ))}
              </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {rest.map(r => (
            <div key={r.title}
              className="rounded-2xl border border-black/[0.06] bg-white hover:shadow-lg hover:border-[#F97316]/20 transition-all hover:-translate-y-0.5 group overflow-hidden">
              <div className="p-5 pb-3">
                <div className="flex items-start justify-between mb-4">
                  <div className="w-10 h-10 rounded-xl flex items-center justify-center"
                    style={{ background: r.tagBg, border: `1px solid ${r.tagColor}25` }}>
                    <r.icon size={16} style={{ color: r.tagColor }} />
                  </div>
                ...
                <div className="text-[14px] font-bold text-[#18181B] mb-1.5">{r.title}</div>
                <div className="text-[12px] text-[#71717A] leading-relaxed">{r.desc}</div>
              </div>
              <div className="mx-5 mb-4 rounded-xl overflow-hidden bg-[#F5F5F4] border border-black/[0.04]">
                {r.mockup.map(m => (
                  <div key={m.label} className="flex items-center justify-between px-3 py-2 border-b border-black/[0.04] last:border-0">
        ...
        <div className="text-center mt-14">
          <p className="text-[14px] text-[#71717A] mb-4">All 12+ reports included in Skylitee Pro</p>
          <Link href="/install" className="inline-block px-8 py-3.5 bg-[#F97316] hover:bg-[#EA580C] text-white font-bold rounded-xl text-[14px] transition-all shadow-[0_0_20px_rgba(249,115,22,0.3)] hover:shadow-[0_0_32px_rgba(249,115,22,0.5)]">
